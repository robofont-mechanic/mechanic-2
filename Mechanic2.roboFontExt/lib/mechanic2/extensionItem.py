...
if __name__ == "__main__":
    ...
    mechanicDataURL = "http://127.0.0.1:8888/api/extensions.json"

    data = getExtensionData(mechanicDataURL)
    ...
    token = "n3gnqfjr5wpoCg9AY8Jq"
    data.append(dict(
        repository="https://gitlab.com/typemytype/tmt-robofont-extensions",
        extensionPath="PixelShadow/PixelChecker.roboFontExt",
        infoPath="https://gitlab.com/typemytype/tmt-robofont-extensions/raw/master/PixelShadow/PixelChecker.roboFontExt/info.plist?private_token=%s" % token,
        zipPath="https://gitlab.com/typemytype/tmt-robofont-extensions/-/archive/master/tmt-robofont-extensions-master.zip?private_token=%s" % token,
    ))

    def checkExtensionItem(r):
        print("remoteVersion:       ", r.remoteVersion(), r.remoteVersion())
        print("extensionNeedsUpdate:", r.extensionNeedsUpdate())
        print("extensionName:       ", r.extensionName())
        print("remoteZipPath:       ", r.remoteZipPath())
        print("isExtensionInstalled:", r.isExtensionInstalled())
        #r.remoteInstall(forcedUpdate=True)
        print()


    for item in data:

        r = ExtensionRepository(item)
        checkExtensionItem(r)

    extensionStoreDataURL = "http://extensionstore.robofont.com/data.json"
    data =getExtensionData(extensionStoreDataURL)

    for item in data:

        r = ExtensionStoreItem(item)
        checkExtensionItem(r)
